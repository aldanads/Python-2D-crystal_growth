...
def KMC(MoS2_lattice, MoS2_crystal,distribution_parameters,events,rng):
    ...
    #l_defect_species = [2 for i in np.arange(len(xy_adatom_edge))]
    #l_defect_species = l_defect_species + [4 for i in np.arange(len(coord_Mo))] # Label of defects (adatoms = 2 and atoms at edge of the cluster = 4)
    """
    ---------------------------------------------------------------------------
    """
